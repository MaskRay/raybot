...
doCommand :: Array Int Command -> BF -> WriterT BS.ByteString IO BF
doCommand cmds bf@(BF _ _ ip) = doCommand' (cmds ! ip) cmds bf
  where
  doCommand' :: Command -> Array Int Command -> BF -> WriterT BS.ByteString IO BF
...
-}
  ...
  doCommand' (JmpBackward n) cmds bf@(BF c cp ip) = {-# SCC "JmpBack" #-} do
    c' <- liftIO $ unsafeRead c cp
    if (c' /= 0)
      then return (BF c cp newInstPtr)
...
loadProgram prog = if snd st /= 0 then error "unmatched" else optimize (cs++[Halt])
  where
  st = runState (mapM decode prog) 0
...
execute2 :: Array Int Command -> BF -> WriterT BS.ByteString IO ()
execute2 cmds bf@(BF _ _ ip) = do
  if cmds ! ip == Halt
    then liftIO $ halt
    else doCommand cmds bf >>= execute2 cmds

execute :: String -> IO String
execute prog = do
  s <- core
  result <- handle (\(SomeException _) -> return "unmatched parentheses") $ liftM (S.decode . BS.unpack) $ execWriterT (execute2 (loadProgram prog) (BF s 0 0))
  return result
